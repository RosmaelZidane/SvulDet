...
#         self.register_buffer('v1', torch.tensor(v1).float())  
...
#         self.v1_projector = nn.Linear(self.v1.shape[0], hidden_feats)
...
#         # Interpolating dimensions (match the embedding feature dimension)
...
class MultiTaskGAT(nn.Module):
    def __init__(self, in_feats, hidden_feats, num_heads, dropout,
        ...
        # Now v1 is a learnable parameter
        self.v1 = nn.Parameter(torch.tensor(v1).float())  # <--- Learnable parameter

        # Small MLP to compute beta from v1
    ...
    def forward(self, g):
        ...
        # Compute beta from learnable v1
        beta = self.compute_beta()
        dynamic_dropout = self.base_dropout * (1 + beta.item())
        dynamic_dropout = min(dynamic_dropout, 0.8)  # cap to avoid crazy dropout
...
def train_with_param_trials(df, graph_dir, config_grid):
    ...
    if os.path.exists(best_mo_path):
        ...
        return  # exit the function after testing

    # No pre-trained model: start training
    best_val_f1 = 0
    ...
    for idx, (hidden, dropout, lr) in enumerate(trials):
        ...
        checkpoint_callback = ModelCheckpoint(
            ...
            filename=os.path.basename("best_model"),  # unified best model name
            save_top_k=1,
            mode='max',
        )
    ...
    # Copy best checkpoint to consistent location
...
# #
# # ----------------- USAGE EXAMPLE for GL constrution -----------------
...
# # graph_list = gl_loader.get_graph_list()
# # gl, _ = GetlistGL.dependency_graph_construction(graph_list, config_grid)
# # glnx = GLs.networkx_to_dgl(nx_graph = gl, feature_size = config_grid['gl_vec_length'], device='cpu')
# # v1 =  GLs.compute_graph_embedding(glnx, method=config_grid['graph_to_vec_method']).numpy()




# # print("\nFinal Function-Level Dependency Graph:")
# # print(f"Nodes: {list(gl.nodes())}")
# # print(f"Edges: {list(gl.edges())}")
# # print(f"Cluster labels: {_}")
